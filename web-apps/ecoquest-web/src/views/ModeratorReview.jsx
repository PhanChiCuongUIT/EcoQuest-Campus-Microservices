...
import {
  ...
  Image as ImageIcon, Eye, FileVideo, FileText,
} from 'lucide-react';
...
function evidenceKind(url) {
  ...
  if (value.startsWith('data:image/') || value.match(/\.(jpeg|jpg|gif|png|webp)(\?|$)/i)) return 'image';
  if (value.startsWith('data:video/') || value.match(/\.(mp4|webm|mov)(\?|$)/i)) return 'video';
  if (value.startsWith('data:application/pdf') || value.match(/\.pdf(\?|$)/i)) return 'document';
  return 'file';
}

function evidenceItems(item) {
  const urls = Array.isArray(item.evidenceUrls) && item.evidenceUrls.length
    ? item.evidenceUrls
    : item.evidenceUrl
      ? [item.evidenceUrl]
      : [];
  return urls.map((url, index) => ({ id: `${item.id}-${index}`, url, src: evidenceSrc(url), kind: evidenceKind(url) }));
}
...
export default function ModeratorReview() {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      toast({
        type: 'error',
        message: err.response?.status === 403 ? 'You cannot review your own action.' : 'Approval failed',
      });
    } finally {
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      toast({
        type: 'error',
        message: err.response?.status === 403 ? 'You cannot review your own action.' : 'Rejection failed',
      });
    } finally {
      setActionLoading(prev => { const next = { ...prev }; delete next[id]; return next; });
    }
  ...
  return (
    <div>
      <Modal
        ...
        titleIcon={selectedEvidence?.kind === 'video'
        ...
        {selectedEvidence && (
          <div className="evidence-viewer">
            {selectedEvidence.kind === 'video' ? (
              <video src={selectedEvidence.src} controls autoPlay style={{ maxWidth: '100%', maxHeight: '70vh' }} />
            ) : (
              <img src={selectedEvidence.src} alt="Evidence Full View" />
            )}
          </div>
        )}
      </Modal>
      ...
      {filteredQueue.length === 0 ? (
        ...
        filteredQueue.map(item => {
          const evidences = evidenceItems(item);
          return (
                    ...
                    {evidences.length ? (
                      <div className="review-evidence-grid">
                        {evidences.map((evidence, index) => {
                          const broken = brokenEvidence[evidence.id];
                          if (evidence.kind === 'image' && !broken) {
                            return (
                              <button
                                key={evidence.id}
                                type="button"
                                className="review-evidence-thumb"
                              ...
                              >
                                <img
                                  ...
                                  onError={() => setBrokenEvidence(prev => ({ ...prev, [evidence.id]: true }))}
                                />
                                <span><Eye size={14} /> Photo {index + 1}</span>
                              </button>
                            );
                          }
                          if (evidence.kind === 'video') {
                            return (
                              <button
                                key={evidence.id}
                                type="button"
                                className="review-evidence-thumb is-video"
                                onClick={() => setSelectedEvidence(evidence)}
                                title="Play evidence video"
                              >
                                <FileVideo size={24} />
                                <span><Eye size={14} /> Video</span>
                              </button>
                            );
                          }
                          return (
                            <a key={evidence.id} className="evidence-fallback" href={evidence.src} target="_blank" rel="noopener noreferrer">
                              {evidence.kind === 'document' ? <FileText size={18} /> : <ImageIcon size={18} />}
