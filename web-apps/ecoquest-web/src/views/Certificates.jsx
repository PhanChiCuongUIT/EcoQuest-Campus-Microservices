...
/* ── Certificate card ─────────────────────────────────────────── */
function CertCard({ cert, user, onPreview, onDownload }) {
  const rank = cert.rankNumber || 1;
  const meta = RANK_META[rank] || RANK_META[3];
  const rankSuffix = { 1: 'st', 2: 'nd', 3: 'rd' }[rank] || 'th';
...
export default function Certificates({ studentId }) {
  ...
  const handleDownload = async (cert) => {
    try {
      ...
      toast({ type: 'success', message: 'Certificate PDF downloaded', sub: 'The file was generated by Recognition service.' });
    } catch (error) {
      toast({
        type: 'error',
    ...
    <div>
      ...
      {certs.length === 0 ? (
        ...
        <div style={{
          ...
          {certs.map(cert => (
            <CertCard key={cert.id} cert={cert} user={user} onPreview={setPreview} onDownload={handleDownload} />
          ))}
        </div>
      )}
