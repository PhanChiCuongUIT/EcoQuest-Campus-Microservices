...
- Moderator có Student self-service cho chính mình và quyền review.
...
- Recognition tạo certificate record, PDF MinIO và download `application/pdf`.
- Reward claim demo.
...
- Report analytics dùng read model riêng consume action accepted/rejected event; có API summary tuần/tháng/năm/all và student analytics.
...
- Report service hiện có analytics read model cho dashboard tuần/tháng/năm/all, nhưng chưa phải analytics warehouse lớn với BI pipeline riêng.
